...
2. **Show it**, cropped to the part that carries the answer. `marker_view.py` handles the
   ...
   **Open it in the browser pane, one question per page**, with
   `marker_view.py <recording> N --page` - or `--page --at 11.7` for a moment with no marker.

   Each page shows **before, the moment, and after**, because what a step accomplished is visible
   in the difference rather than in one picture; the captions carry each neighbour's offset
