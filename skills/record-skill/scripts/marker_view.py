...
def main() -> None:
    ...
    if args.html:
        print('<div style="padding:1rem 0"><div style="background:var(--surface-1);'
              'border-radius:12px;padding:12px">'
              f'<img src="data:image/jpeg;base64,{b64}" alt="screen at marker {args.index}" '
              'style="width:100%;display:block;border-radius:8px;'
              'border:0.5px solid var(--border)">'
              f'<p style="margin:10px 2px 0;font-size:13px;color:var(--text-secondary)">'
              f'marker {args.index} &middot; {m["t"] / 1000:.1f}s &middot; {m["file"]}</p>'
              '</div></div>')
    else:
